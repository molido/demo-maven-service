...
def HttpReq(reqType,reqUrl,reqBody){
    def gitServer = "http://10.67.47.95/api/v4"
    def result
    withCredentials([string(credentialsId: '95', variable: 'gitlabToken')]) {
...
def ChangeCommitStatus(projectId,commitSha,status){
    def commitApi = "projects/${projectId}/statuses/${commitSha}?state=${status}"
    def response = HttpReq('POST',commitApi,'')
    println(response)
    return response
}
